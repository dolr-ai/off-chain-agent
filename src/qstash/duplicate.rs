...
impl VideoHashDuplication {
    ...
    async fn store_user_uploaded_content_approval(
        ...
        let is_approved = if ai_detector.is_configured() {
            log::info!("Running AI detection for video {} with Storj URL", video_id);

            match ai_detector.detect_video(&storj_url).await {
                Ok(response) => {
                    log::info!(
                ...
                Err(e) => {
                    log::error!(
                        "AI detection failed for video {} with Storj URL: {}. Sending to manual review.",
                        video_id,
                        e
                    );
